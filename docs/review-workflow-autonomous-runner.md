...
Three guarantees earned by a real ~60-minute implementation that committed its work and
then could not be recovered:

- **A long call is not corruption.** The Claude CLI repeats `init` AND `result` at session
  refresh/compaction boundaries; the parser accepts both (the last `result` wins — final
  text, usage, stop reason). The real corruption signal — a result from a session no init
  ever announced, or a malformed result — still fails the call.
- **A failed invocation keeps its evidence.** Every failure persists whatever the CLI
  emitted (capped and secret-redacted, like successful captures) plus the failure message —
