...
export const CONFIG_TOOL_DEFINITIONS = [
  {
    name: 'codemoot_docs',
    description:
      'Read the canonical CodeMoot documentation. ALWAYS read topic "configuration" before creating or editing a .cowork.yml, "autonomous-runner" before driving codemoot workflow commands, and "handoff-contracts" before authoring or debugging any agent JSON response (a wrong envelope field costs a full invocation). Adapter kinds are claude, codex and cursor — for cursor, read "configuration" first: it is a router (one CLI, many vendors), its effort level is part of the model id, and a headless run without --force silently produces nothing. A workflow can also run a plan VERBATIM (--plan-as-is / reviewGated.planAsIs: no refinement rewrite, no plan-review gate) — read "autonomous-runner" first: batches come from the plan\'s own "Batch N" headings and verification runs the plan\'s OWN fenced sh commands, so a plan that declares none gets only a minimal worktree check. `codemoot relay` is a DIFFERENT command from `codemoot workflow` — a message bus that never rewrites the plan (so it has no --plan-as-is and needs none) — read topic "relay" before naming any relay argument: it carries the complete reference for run/resume/pause/status/log, and relay `--batches` overrides the parsed batch COUNT, which is not the workflow\'s --max-batches. No MCP tool starts, resumes or reads a relay run; it is command-line only.',
    inputSchema: {
